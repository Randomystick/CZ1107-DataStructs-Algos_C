...
//////////////////////////////
    //empty ll
    if ((*ptrHead) == NULL || (*ptrHead)->num == NULL)
    {
        return -1;
    }
...
//////////////////////////////
    ...
    while (cur != NULL)
    {
        ListNode* item = findNode(head2, offset);
        insertNode(ptrHead1, sizeOf+offset, item->num);
        offset++;
        cur = cur->next;
    }
...
//////////////////////////////
    ...
    // as long as one head still has stuff...
    do
    {
        if (whoseHead == 0) //let's do on head1
        {
            if (cur1 != NULL)
            {
                insertNode(ptrHead, ptrHeadIndex++, cur1->num);
                cur1 = cur1->next;
            }
            else // head1 has no more nodes
            {
                head1Empty = 1;
            }
        }
        else //let's do on head2
        {
            if (cur2 != NULL)
            {
                insertNode(ptrHead, ptrHeadIndex++, cur2->num);
                cur2 = cur2->next;
            }
            else // head2 has no more nodes
            {
                head2Empty = 1;
            }
        }
        whoseHead = !whoseHead;
    } while (head1Empty == 0 || head2Empty == 0);
...
//////////////////////////////
    ...
    //insertNode
    cur = pree->next;
	DblListNode* newNode = malloc(sizeof(DblListNode));
	newNode->num = value;
