...
//////////////////////////////
    ...
    // If tree is not empty, then Enqueue root to the Queue
        // i have a pointer to BSTNode
        // i need a pointer to QueueNode for isEmpty
            // QueueNode contains a pointer to BSTNode
        // hence, i need to obtain a pointer to my pointer to BSTNode.
    if (temp != NULL) //(!isEmpty(&root))
    {
        enqueue(&q.head, &q.tail, temp);
    }

    // Repeat until queue is empty
        // isEmpty needs a pointer to QueueNode
        // Queue has a pointer to QueueNode
    while (!isEmpty(q.head))
    {
        // Dequeue node's data from the queue and print it
        temp = dequeue(&q.head, &q.tail);
        printf("%i ", temp->item);

        if (temp->left != NULL)
        {
            // Enqueue node's left child to the q
            enqueue(&q.head, &q.tail, temp->left);
        }

        if (temp->right != NULL)
        {
            // Enqueue node's right child to the q
